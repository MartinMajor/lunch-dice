...
export default function GroupSwitcher({ currentGroupId, currentGroupName }: Props) {
  ...
  return (
    <>
        ...
        {open && (
          <div className="absolute right-0 top-full mt-2 w-64 bg-[#111118] border border-gold/20
            ...
            <div className="border-b border-gold/10">
              ...
              <button
                onClick={() => { setOpen(false); setMode("creating"); }}
                className="w-full text-left px-4 py-2.5 text-cream/70 text-sm
                           hover:bg-white/5 hover:text-cream transition-colors"
              >
                + Create new group
              ...
              <button
                onClick={() => { setOpen(false); setMode("joining"); }}
                className="w-full text-left px-4 py-2.5 text-cream/70 text-sm
                           hover:bg-white/5 hover:text-cream transition-colors"
              >
                ↵ Join with a link
      ...
      {/* Modal for creating / joining */}
      {mode !== "idle" && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-casino-black/80"
          onClick={closeAll}
        >
          <div
            className="bg-[#111118] border border-gold/20 rounded-lg shadow-xl w-full max-w-sm p-6 flex flex-col gap-4"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="font-display text-gold tracking-wider text-sm uppercase">
              {mode === "creating" ? "New Group" : "Join Group"}
            </h2>
              ...
              <form onSubmit={handleCreate} className="flex flex-col gap-3">
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 bg-gold text-casino-black font-display tracking-widest text-sm py-3 rounded
                  ...
                  <button
                    type="button"
                    onClick={closeAll}
                    className="flex-1 bg-white/5 text-cream/70 text-sm py-3 rounded
                               hover:bg-white/10 hover:text-cream transition-colors"
                  >
                    Cancel
                  </button>
                </div>
